...
from config import directory_names, path_to_directories, save_directory, boundary_names, boundary_region_to_mesh_map, \
    ctu_len, kinvis, path_to_mesh, path_to_mesh_boundary, meshname, wallunits_ref_name
...
def compute_closest_point(row, df_coords):
    """
    Computes the closest point in a DataFrame of coordinates to a given row based on 3D Euclidean
    distance while considering specific conditions. The function calculates scaled distances
    (`xplus`, `yplus`, `zplus`) based on friction velocity (`ustar`) and kinematic viscosity.

    Parameters
    ----------
    row : pd.Series
        A row from a DataFrame containing the 'x', 'y', 'z', and 'dist' values representing the
        position and distance variables of the point in question.
    df_coords : pd.DataFrame
        A DataFrame containing coordinate points and variables, including columns: 'x', 'y',
        'z', 'dist', and 'Shear_mag'.

    Returns
    -------
    pd.Series
        A pandas Series containing three scaled distances: `xplus` (scaled x-distance),
        `yplus` (scaled distance based on `dist`), and `zplus` (scaled z-distance),
        relative to the closest point in the DataFrame.
    """
...
if __name__ == "__main__":
    ...
    for dirname, dir_color in zip(directory_names, TABLEAU_COLORS):
        ...
        file_path = path_to_mesh.replace(f"{meshname}.xml", wallunits_ref_name)
        ...
        for bname, b_color in zip(boundary_names, TABLEAU_COLORS):
            ...
            surfdist_file = f"{meshname}-surfdistance_{bname}"
            surfdist_file_path = path_to_mesh.replace(f"{meshname}.xml", surfdist_file)
            ...
            if showGeometry:
                ...
                df_out.to_csv(boundary_file_path.replace("wss", "wallunits") + ".csv", sep=' ', index=False)
